...
def display_result(result: tuple[list[str], int]):
    ...
    table.add_column("Minutes", style="bright_yellow", footer="[bright_yellow]" + str(result[1]))
    for index, i in enumerate(result[0]):
        table.add_row(
            " ".join(left),
            ("<-- " if index % 2 == 1 else "") + 
            (" ".join([ii[0].upper() for ii in i[4:].split(" + ")]) if index % 2 == 0 else i[4].upper()) +
            (" -->" if index % 2 == 0 else ""),
            " ".join(right),
            str(slowest_time(i[4:]) if index % 2 == 1 else slowest_time(*(i[4:].split(" + "))))
        )
        if index % 2 == 0:
...
def recursive():
    successes = []
    fails = []
    def func(time: int, at_dock: bool, right: list[str], left: list[str], chain: list) -> bool:
        if time > MAX_TIME:
            fails.append((chain, sum([(slowest_time(i[4:]) if index % 2 == 1 else slowest_time(*(i[4:].split(" + ")))) for index, i in enumerate(chain)])))
            return
        if len(left) == 4:
            successes.append((chain, sum([(slowest_time(i[4:]) if index % 2 == 1 else slowest_time(*(i[4:].split(" + ")))) for index, i in enumerate(chain)])))
            return
        if at_dock:
            for person1 in right:
                for person2 in right:
                    if person1 != person2:
                        ...
                        func(time + slowest_time(person1, person2), False, tempright, templeft, tempchain)
    ...
    print(display_result(successes[__import__("random").randint(0, len(successes) - 1)]))
    print(f"{len(successes)}/{len(successes) + len(fails)} combinations were successful.")
